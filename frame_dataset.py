...
class FrameDataset(Dataset):
    ...
    def __len__(self):
        files = glob.glob(TRAIN_DATA_PATH + '/*.jpg')
        return len(files) - 2

    def __getitem__(self, idx):
        #print(idx)
        img1 = Image.open(os.path.join(TRAIN_DATA_PATH, FILE_TEMPLATE.format(idx)))
        img2 = Image.open(os.path.join(TRAIN_DATA_PATH, FILE_TEMPLATE.format(idx+1)))
        img3 = Image.open(os.path.join(TRAIN_DATA_PATH, FILE_TEMPLATE.format(idx+2)))
